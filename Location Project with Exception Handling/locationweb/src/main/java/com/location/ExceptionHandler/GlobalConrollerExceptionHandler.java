...
public class GlobalConrollerExceptionHandler {
	...
	public ResponseEntity<ErrorDetails> handleValidationException(ValidationException validationException) {
		...
		errorDetail.setStatusCode(HttpStatus.ACCEPTED.value());
		
		ResponseEntity<ErrorDetails> response = new ResponseEntity<ErrorDetails>(errorDetail, HttpStatus.ACCEPTED);
